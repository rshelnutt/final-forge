...
The Claude triage workflow will apply platform labels (`website`, `mobile`, `ios`, `android`) and feature area labels (`binder-builder`, `deck-builder`, etc.) based on dropdown selections and issue content.
...
### 4. Card Data Issue (`card_data.yml`)
...
- **Concurrency:** One triage job per issue at a time
- **Behavior:**
  ...
  - On follow-up comments, re-evaluates if new info changes the categorization
...
- **Behavior:**
  ...
  - If potential duplicates found, posts a comment listing them with links
...
- **Behavior:** Posts a comment explaining closure, links to the original issue if referenced
...
- **Condition:** Issue is open, has `autoclose` label, commenter is not a bot
- **Action:** Removes the `autoclose` label

### 13. Non-Write Users Check (`non-write-users-check.yml`)

- **Trigger:** `pull_request` with changes to `.github/**`
- **Action:** If PR modifies `allowed_non_write_users` in any workflow file, posts a warning comment about security implications
- **Idempotent:** Won't post duplicate comments

### 14. Issue Sweep (`sweep.yml`)

- **Trigger:** Twice-daily cron (`0 10,22 * * *`), `workflow_dispatch`
- **Action:** Enforces lifecycle timeouts
- **Behavior:**
  ...
  - Issues labeled `triage` with no activity after 30 days → add `needs review` label
